...
# ORDER_DIRECTION类定义了订单的买卖方向，包括股票的买入和卖出，期货的多开和空开，以及申购等
...
# TIME_CONDITION：定义了订单的时间条件，包括立即完成，否则撤销（IOC），本节有效（GFS），当日有效（GFD），指定日期前有效（GTD），撤销前有效（GTC），集合竞价有效（GFA）等
